...
def calculate_strat2_all_variables(c_max, delta_m_commute, nu_w_work, nu_m_move, nu_min, nu_m_charge, nu_m_transfer, xi, tau, zeta, n_m=1):
    c_m_max = tau * c_max # chargers capacity is tau times the workers capacity

    # extract delta_m_rest for case 2
    delta_m_rest_case_2 = np.maximum(0, c_max/(nu_w_work+nu_min)*(1-nu_min/nu_m_charge)-2*delta_m_commute*(1+nu_m_move/nu_m_charge) - c_max/nu_m_charge * (zeta * nu_m_transfer + nu_min) / (nu_m_transfer * xi - nu_min))
    ...
    c_m_charged = np.maximum(0, np.minimum(c_m_max, 2 * (nu_m_move + nu_min) * delta_m_commute + c_max * (zeta * nu_m_transfer + nu_min) / (nu_m_transfer * xi - nu_min) + nu_min * delta_m_rest_case_2))
    ...
    delta_m_rest_case_1 = np.maximum(0, (c_m_charged - 2 * (nu_m_move + nu_min) * delta_m_commute)/nu_min - (c_m_charged/(nu_m_charge - nu_min) * nu_m_charge - 2 * delta_m_commute * nu_m_move) / (nu_min + nu_min**2/(nu_w_work + nu_min) * (nu_m_transfer * xi - nu_min)/(zeta * nu_m_transfer + nu_min)))
    #print(f"delta_m_rest_case_1: {delta_m_rest_case_1}")
    delta_transfer = np.maximum(0, np.minimum(c_max /(xi * nu_m_transfer - nu_min), (c_m_charged - 2 * (nu_m_move + nu_min) * delta_m_commute)/(zeta * nu_m_transfer + nu_min) - nu_min/(zeta * nu_m_transfer + nu_min) * delta_m_rest_case_1)) # Either the time the charger needs to give the worker its remaining capacity, or the time it needs to charge all workers to the maximum capacity (considering that there is a energy loss) 
    print(f"delta_transfer: {delta_transfer}")
    c_w_charged = (xi * nu_m_transfer - nu_min) * delta_transfer # the max boundary for c_w_charged is already encoded in delta_transfer, so we can just multiply it with the rate at which the charger can transfer energy to the worker
    n_w = zeta * n_m 
    ...
    duty_cycle = delta_w_work/((delta_transfer + delta_w_work + delta_w_rest + delta_m_charge + 2 * delta_m_commute + delta_transfer + delta_m_rest)/2) # divide by 2 because it should be the same but we need the chargers variables in there; only calculate for a single worker
